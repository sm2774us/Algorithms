const MaxHeap = () => {
    ...
    const pairIsInOrder = (v1, v2) => {
        ...
        if(res === 0) // if values equal, sort alphabetically
    ...
    const heapifyUp = () => {
		...
		while (hasNext(index) && !pairIsInOrder(heapContainer[index-1], heapContainer[index])) {
				swap(index, index - 1);
				index = index - 1;
			}
    }
...
var topKFrequent = function(words, k) {
    ...
    Object.keys(freqMap)
        .forEach(key => 
                 heap.add({ key, val: freqMap[key] })
        );
    
    // return top k
    return heap.getItems().slice(0,k).map(res => res.key);
};