...
import {
  ...
  topKFrequentElementsUsingBucketSort,
} from "./top_k_frequent_elements.ts";

test("347. topK Frequent Elements Using Max Heap - TC: O(K*log(N)) ; SC: O(N)", () => {
  assertEquals(topKFrequentUsingMaxHeap([1, 1, 1, 2, 2, 3], 2), [1, 2]);
  assertEquals(topKFrequentUsingMaxHeap([1], 1), [1]);
  assertEquals(
    topKFrequentElementsUsingBucketSort([1, 1, 1, 2, 2, 3], 2),
    [1, 2]
  );
